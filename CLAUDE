...
|---|---|---|---|---|---|
| 1 | Network and Device | Shared device fingerprint, host and guest | 7 | 25%+ | High confidence |
| 2 | Network and Device | Cross-dinner device fingerprint match | 7 | Any | Must combine with at least one other signal to score. Standalone is a watch flag only -- surfaces in Weekly Insights but does not flag individual hosts. High confidence when combined with other signals. |
| 3 | Network and Device | Same device fingerprint across guests | 5 | 25%+ | Must combine with at least one other signal to score. Standalone is a watch flag only -- does not score. |
